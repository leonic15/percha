...
- **Entonces** veo un indicador de progreso y al finalizar se muestran las sugerencias de IA para que las revise y apruebe (igual que en PERCHA-015)
...
- Reutilizar el mismo endpoint `POST /api/prendas/analizar` de PERCHA-014
- En este flujo, la imagen ya está en Supabase Storage: pasarle la URL de la imagen existente al endpoint
- Al completar el análisis, abrir el formulario de edición (PERCHA-011) pre-completado con las nuevas sugerencias
- Actualizar `ia_analizada = true` y `ia_descripcion` en la tabla `prendas` al guardar el resultado
- Registrar el uso en `ai_usage` igual que en PERCHA-014 (para futura facturación/límites)
...
- [PERCHA-014] Análisis automático de prenda con IA al agregar (comparte endpoint y lógica)
- [PERCHA-015] Revisión y aprobación de sugerencias de IA (comparte pantalla de revisión)
- [PERCHA-010] Ver detalle de prenda (punto de entrada)
- [PERCHA-011] Editar prenda (flujo de aprobación desemboca en formulario de edición)
