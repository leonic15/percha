...
- **Entonces** el generador funciona normalmente, pero el botón "Vestir mi look" (LOOKSI-035) aparece deshabilitado con tooltip "Completá tu perfil para usar esta función"
...
- Los campos `altura_cm` y `peso_kg` se usan en LOOKSI-035 para generar proporciones realistas
...
- [LOOKSI-005] Edición de perfil básico (comparte tabla `profiles` y la pantalla de configuración)
- [LOOKSI-017] Generar look desde cero (consume `genero` en el prompt)
- [LOOKSI-035] Vestir mi look (consume `altura_cm`, `peso_kg` y `genero` para la imagen)
