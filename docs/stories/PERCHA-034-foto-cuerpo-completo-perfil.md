...
Como **usuario autenticado**, quiero **subir una foto de cuerpo completo en mi perfil** para **que la app pueda generar imágenes fotorrealistas de cómo me queda un look (PERCHA-035)**.
...
- **Entonces** la imagen se previsualiza, se comprime en el cliente y se sube a Supabase Storage. Veo una confirmación de éxito y el botón "Vestir mi look" (PERCHA-035) se habilita
...
- La URL del body photo se necesita en el endpoint `POST /api/looks/generar-imagen` (PERCHA-035). Generar una signed URL fresca en el servidor antes de pasarla a Google Imagen 3
- Migración: incluir en `supabase/migrations/20260527000001_profiles_datos_corporales.sql` (junto con PERCHA-033) o migración propia
...
- [PERCHA-033] Datos corporales en perfil (comparte pantalla y migración)
- [PERCHA-035] Vestir mi look (consume `body_photo_url`)
