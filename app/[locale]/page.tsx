...
// ── Color swatches para garment images ───────────────────────────────────────
// Fuente: components.jsx del prototipo LookSi.html
...
// ── GarmentImage — placeholder de foto de prenda ─────────────────────────────
// Stripes diagonales sobre color sólido + etiqueta inferior.
// Equivalente a <GarmentImage /> del prototipo (sin el card wrapper).
...
function GarmentImage({
  ...
  // ID único por color + label para evitar conflictos de SVG pattern en la misma página
  const safeLabel = (label ?? color).replace(/\W+/g, "_");
  const patternId = `gp_${color}_${safeLabel}`;

  // Safari/WebKit bug: aspect-ratio no se respeta en elementos position:absolute
  // sin alto explícito — el navegador usa el alto del bloque contenedor en vez de
  // derivarlo del ancho. Solución: calcular el alto explícito cuando el ancho es
  // un número en px.
  const numericWidth = typeof style.width === "number" ? style.width : undefined;
  ...
  return (
    <div
      ...
      style={{
        ...
        /* overflow:hidden + transform:rotate() = bug iOS Safari: el contenido
           desaparece por compositing layers. Solo lo necesitamos para el SVG. */
    ...
    >
      {src ? (
        /* ── Foto real (PNG con fondo transparente) ─────────────────── */
        ...
        /* ── Placeholder SVG (stripes diagonales) ──────────────────── */
        <svg
          width="100%"
          height="100%"
